...
User.prototype = {
  ...
  add: function(item, callback) {
    ...
    var hash = get_hash(item.password);
    item.password = hash;
    ...
    this.model.find( function (err, users) {
      if (users) {
        ...
        for (var i = 0; i < users.length; i++ ) {
          ...
          for ( var i1 = 0; i1 < request.auth.credentials.companyId.length; i1++ ) {
            if ( users[i].companyId.indexOf( request.auth.credentials.companyId[i1] ) !== -1 ) {
              console.log('users[i].companyId = ', users[i].companyId);
              console.log('request.auth.credentials.companyId[i1] = ', request.auth.credentials.companyId[i1] );
              foundCompany = true;
            }
          } 
