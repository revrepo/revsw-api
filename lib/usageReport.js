...
var sumAccounts_ = function( records, accountID ) {
  ...
  for ( var id in lastsForAcc ) {
    ...
    res.ssl_certs = lastsForAcc[id].ssl_certs;
    console.log('lastsForAcc[id].ssl_names',id,lastsForAcc[id].ssl_names);
    res.ssl_names = lastsForAcc[id].ssl_names;
  }
 ...
 *     }
 *   },
 *   ssl_certs: { total: 000, deleted: 000, active: 000 },
 *   ssl_names: 000,
 *   api_keys: {
 *       total: 000,
 *       inactive: 000,
  ...
  return promise.resolve( day )
    ...
    .then( function( responses ) {
      ...
      _.forEach( accDomainsCount, function( item, aid ) {
        ...
        accounts[aid].ssl_names = accSSLEnabled[aid] || 0;
        console.log(aid,accSSLEnabled[aid]);
...
var checkThenCollect_ = function( to, accountID ) {
  ...
  console.log('checkThenCollect_::to < today', to ,  moment(to).format().unix(),today,(to < today),( moment(to).format()<today));
...
module.exports.checkLoadReports = function( from, to, accountID, onlyOverall, keepSamples ) {
  ...
  var recollected;
  // TODO: use moment for conver String to Date
