...
var sumAccounts_ = function( records, accountID ) {
  ...
  var res = {
    ...
    platforms: PLATFORMS_DEFAULT_DATA_OBJECT,
  ...
  for ( var id in lastsForAcc ) {
    console.log('id_,lastsForAcc',id,lastsForAcc);
    accumulate_( res.domains, lastsForAcc[id].domains );
    accumulate_( res.applications, lastsForAcc[id].applications );
    accumulate_( res.platforms.Android, lastsForAcc[id].platforms.Android );
    accumulate_( res.platforms.Windows_Mobile, lastsForAcc[id].platforms.Windows_Mobile );
    accumulate_( res.platforms.iOS, lastsForAcc[id].platforms.iOS );
      ...
      return ( accountID ? promise.resolve( accountID ) : accountsModel.allHalfDeadIDs( day ) )
        ...
        .then( function( domains ) {
          return promise.all([
            ...
            applicationsModel.accountPlatformsData( accountID )
          ]);
        });
    })
...
module.exports.loadReports = function( from, to, accountID, onlyOverall, keepSamples ) {
  ...
  return promise.all([
    ...
    .then( function( resp ) {
      ...
      // check exists Platforms property
      reportsList.forEach( function( acc ) {
        if ( !acc.platforms ) {
          acc.platforms =  PLATFORMS_DEFAULT_DATA_OBJECT;
          return;
        }
      });
