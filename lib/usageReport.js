...
var memoryCache = cacheManager.caching({
  store: 'memory',
  max: config.get('cache_memory_es_max_bytes'),
  ttl: config.get('cache_memory_es_ttl_seconds')/*seconds*/,
...
var PATTERN_SKIP_DOMAIN_NAME_IP_PORT_ = /^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\.){3}([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])(:[0-9]{0,9})?$/;
//  ----------------------------------------------------------------------------------------------//
 ...
 */
var collectWebTrafficData_ = function (day, domains_) {
  ...
  var cacheKey = 'collectWebTrafficData_:start:' + start + ',end:' + end; //+',domains:' +JSON.stringify(domains);
  return promise.resolve()
    .then(function () {
      return multiCache.wrap(cacheKey, function () {
        isFromCache = false;
        var query = {
          size: 0,
          query: {
            ...
            domains: {
              ...
              aggs: {
                original_image_size_bytes: {
                  sum: { field: 'ie_bytes_o' }
                },
                optimized_image_size_bytes: {
                  sum: { field: 's_bytes' }
                }
              }
            }
        ...
        //  NOTE: old code - we not use this filter because we cache response for a time period
        ...
        return es.getClient().search({
          ...
          .then(function (resp) {
            ...
            var traff = [];
            if (ieData.aggregations && ieData.aggregations.domains.buckets.length) {
              // clean and prep imageEngine query for merge
              ieData.aggregations.domains.buckets = _.map(ieData.aggregations.domains.buckets, function (item) {
                var newItem = _.clone(item);
                newItem.optimized_images_count = item.doc_count;
                newItem.optimized_image_size_bytes = item.optimized_image_size_bytes.value;
                newItem.original_image_size_bytes = item.original_image_size_bytes.value;
                delete newItem.doc_count;
                return newItem;
              });
            }
            if (resp.aggregations && resp.aggregations.domains.buckets.length) {

              // merge ImageEngine query to our traffic query
              resp.aggregations.domains.buckets = _.map(resp.aggregations.domains.buckets, function (dom) {
                return _.assign(dom, _.find(ieData.aggregations.domains.buckets, { key: dom.key }));
              });
              //  return promise.map(resp.aggregations.domains.buckets,function( domain ) {
              traff = resp.aggregations.domains.buckets.map(function (domain) {
                var d_ = {};

                // imageEngine
                if (domain.original_image_size_bytes) {
                  d_.image_engine = {
                    original_image_size_bytes: domain.original_image_size_bytes,
                    optimized_image_size_bytes: domain.optimized_image_size_bytes,
                    optimized_images_count: domain.optimized_images_count
                  };
                }
        ...
        .then(function (response) {
          if (isFromCache === true) {
            logger.debug('collectWebTrafficData_:return cache for key - ' + cacheKey);
          }
          return response;
        });
    })
      ...
      if (domains_) {
        if (domains_.length > 0) {
          response = _.filter(response, function (item) {
            return _.contains(domains_, item.domain);
          });
        } else {
          response = [];
        }
 ...
 * image_engine: {
      optimized_image_size_bytes: 0,
      optimized_images_count: 0,
      original_image_size_bytes: 0
    }
 * }
 */
var sumAccounts_ = function( records, accountID ) {
  ...
  records.forEach( function( acc ) {
    ...
    for ( var i in acc.domains_usage ) {
      ...
      if (dmn.image_engine) {
        accumulate_(res.image_engine, dmn.image_engine);
      }
    }
    //  non-falsy accountID, collect domain traffic
    if ( accountID ) {
 ...
 *
 *  @param {[Object{domain,count}]} - distinct list of data domains collected from the ES
 *  @returns {promise(array[string])}
 */
var collectOrphans_ = function( esDomainsData ) {
  ...
  return collectDomains_()
    .then( function( results ) {
      ...
      esDomainsData.forEach( function( itemESData ) {
        if ( domains[itemESData.domain] ) {
          return;
        }
        for ( var i = 0, len = wildcards.length; i < len; ++i ) {
          if ( wildcards[i].test( itemESData.domain ) ) {
            return;
          }
        }
        // NOTE: Omit all requests with IP address in the domain name
        var domainIP = PATTERN_SKIP_DOMAIN_NAME_IP_PORT_;
        if ( domainIP.test( itemESData.domain ) ) {
          return;
        }
        orphans.push( itemESData );
      });

      if ( orphans.length ) {
        orphans = _.uniq(orphans,'domain');
        var orphansDomains = _.map(orphans,'domain');
        logger.warn( 'usageReport.js:collectOrphans_ found orphaned domains: [' + orphansDomains.join(',') + '].' );
      }
      return orphans;
    })
    .catch( function( err ) {
 ...
 *  @returns {promise()}
 */
var mailOrphans_ = function( orphansData ) {
  ...
  var orphanedList = '[empty]';
  if(orphansData.length>0){
    orphanedList='';
    _.forEach(orphansData,function(item, key){
      orphanedList += (key+1)+ '. '+item.domain +' - ' + item.count + '\n\n';
      return;
    });
  }
...
/** *********************************
 *  send email with the domains list and not registered in Location collection billing zones
 *  if [report_about_unknown_billing_zones] config key is not set - it does nothing
 *
 *  @returns {Promise}
 */
var mailAboutUnknownBillingZones_ = function(listUnknownBillingZones){

  if ( !config.report_about_unknown_billing_zones ) {
    logger.warn( 'usageReport.js:mailAboutUnknownBillingZones_, config parameter report_about_unknown_billing_zones is not set');
    return promise.resolve( false );
  }

  // billing zone not found for domain in Location collection
  var sendInformationData = '[empty]';
  if(listUnknownBillingZones.length>0){

    sendInformationData='Domain Name | Unknown Zone Code\n\n';
    _.forEach(listUnknownBillingZones,function(item, key){
      sendInformationData += (key+1)+ '. '+item.domain_name +' - ' + item.unknown_zone_code + '\n\n';
      return;
    });
  }
  //  TODO: edit email body
  var mailOptions = {
    to: config.report_about_unknown_billing_zones,
    subject: 'Report about sites without assigned billing zone',
    text: 'Hi buddy,\n\n' +
      'List of domains  without assigned billing zone:\n\n' + sendInformationData + '\n\n' +
      'Have a nice day,\nCDN Report Bot\n'
  };

  return mail.sendMail( mailOptions )
    .catch( function( err ) {
      logger.error( 'usageReport.js:mailAboutUnknownBillingZones_ error: ' + err.toString() );
      logger.error( err.stack );
      throw err;
    });
};
...
module.exports.collectDayReport = function( day, accountID, dry, orphans ) {
  ...
  return promise.resolve()
    ...
    .then( function( d ) {
      ...
      return ( accountID ? promise.resolve( accountID ) : accountsModel.allHalfDeadIDs( day ) )
        ...
        .then( function( domains ) {
          ...
          return promise.all([
            collectWebTrafficData_( day, domains ), // 0
            domainsModel.accountDomainsData( accountID, day ),// 1
            locationsModel.code2BillingZoneMapping(),// 2
            applicationsModel.accountAppsData( accountID ), // 3
            apiKeysModel.accountAPIKeysData( accountID ),// 4
            usersModel.countUsers(), // 5
            sslNameModel.accountNamesCount( accountID ),// 6
            domainsModel.accountDomainsCount( accountID ), //7
            applicationsModel.accountAppsPerPlatformData( accountID, day ),// 8
            sslCertModel.accountSSLCertificateData( accountID, day ), // 9
            sslCertModel.accountSSLCertificatePerTypeData( accountID, day ), // 10
            // domainsModel.accountDomainsData( accountID, day ),
            dnsZoneModel.accountDNSZonesCount(accountID), // 11
            purgeJobModel.accountPurgeJobsCount(accountID), // 12
            logShippingJobModel.accountLogShippingJobsCount(accountID), // 13
          ]);
        });
    })
    ...
    .then(function (responses) { // jshint ignore:line
      ...
      responses[0/*web traffic*/].forEach( function( domainTrafficData ) {
        var domain = domainTrafficData.domain;
        var accID = accDomains.domain_to_acc_id_map[domain];
        ...
        if ( !acc.domains_usage[domain] ) {
          acc.domains_usage[domain] = {
            traffic_per_billing_zone: {},
            port_hits: domainTrafficData.ports,
            cache_hits: domainTrafficData.cache_hits,
          };
          ...
          if (domainTrafficData.image_engine) {
            acc.domains_usage[domain].image_engine = domainTrafficData.image_engine;
            acc.image_engine.optimized_image_size_bytes += domainTrafficData.image_engine.optimized_image_size_bytes;
            acc.image_engine.original_image_size_bytes += domainTrafficData.image_engine.original_image_size_bytes;
            acc.image_engine.optimized_images_count += domainTrafficData.image_engine.optimized_images_count;
          }
        }
        ...
        domainTrafficData.zones.forEach( function( z ) {
    ...
    // TODO: delete - the orphans report was rebase to reportDomainOrphans
    ...
    .catch( function( err ) {
      ...
      logger.error( msg );
      logger.error( 'Failed usage report for account ID ' + accountID + ' and day ' + day );
      logger.error( err.stack );
      return { error: msg };
    });
};
/**
 * @name reportDomainOrphans
 * @description method for make report about Orphans Domains in system
 *
 * @param {*} options
 * @param {*} cb
 */
module.exports.reportDomainOrphans =  function (options,cb){
  var day;
  var listDomainsWithWebTraffic = [];
  options = _.merge({
    date: 'now',
    dryRun: true,
    verbose: false
  ...
  promise.resolve()
  .then(function(){
    return promise.resolve( options.date );
  })
  .then( parseDay_ )
  .then( function( d ) {
    day = d;
  ...
  // NOTE: get data abount all Web Traffic by day
  .then(function(){
    return collectWebTrafficData_(day)
      .then(function(dataWebTraffic){
        dataWebTraffic.forEach( function( domainData ) {
          listDomainsWithWebTraffic.push( {domain: domainData.domain, count: domainData.count});
        });
        return;
      });
    })
  .then(function(){
    logger.info('reportDomainOrphans:start check orphans');
    return collectOrphans_( listDomainsWithWebTraffic )
      .then( function( foundOrphans ) {
        //  concat 2 orphans lists, remove doubles and mail
        foundOrphans = _.sortByOrder( foundOrphans, ['count','domain'], ['desc','asc']);
        if(options.verbose === true){
          logger.debug('found Orphans'+ JSON.stringify(foundOrphans));
        }
      ...
      .then( function() {
        logger.info('reportDomainOrphans:Success checked orphans');
        return;
      });
  })
...
/**
 * @name reportAboutSitesWithoutAssignedBillingZone
 * @description method for make report about sites without assigned billing zone (not registered in Location collection)
 *
 * @param {Object} options  {date: String|null,dryRun:Bollean,verbose:Boolean}
 * @param {Function} cb
 */
module.exports.reportAboutSitesWithoutAssignedBillingZone = function(options,cb){
  var day;
  var listBillingZones = {};
  options = _.merge({
    date: 'now',
    dryRun: true,
    verbose: false
  ...
  .then(function(){
    return locationsModel.code2BillingZoneMapping()
      .then( function( listBillingZones_ ) {
        listBillingZones = listBillingZones_;
        // delete listBillingZones['TESTSJC20']; // TODOL delete this line
        return;
      });
  })
  // NOTE: get data abount all Web Traffic by day
  .then(function(){
    return collectWebTrafficData_(day)
      .then(function(dataWebTraffic){
        var listUnknownZones = [];
        dataWebTraffic.forEach( function( domainData ) {
          var  domainName = domainData.domain;
          domainData.zones.forEach( function( z ) {
            var zone = listBillingZones[z.code];
            if(!zone) {
              listUnknownZones.push({domain_name:domainName,unknown_zone_code: z.code});
              if(options.verbose === true){
                logger.debug('found Unknown Billing Zone: '+ z.code);
              }
            }
          });
        });

        if(listUnknownZones.length > 0 && (!options.dryRun || options.dryRun === false)){
          return mailAboutUnknownBillingZones_( listUnknownZones );
        }
        return listUnknownZones;
      });
    })
  .then(function(result){
    cb(null,result);
...
var checkThenCollect_ = function( to, accountID ) {
  ...
  if ( to < today ) {
    ...
    return promise.resolve(false);// NOTE: THIS LINE DISABLED FEATURE COLLECT data for current day - it kills ES - All data will be load from storage
    // NOTE: NOTE DON't DELETE THIS CODE - IT CAN BE NEEDED
    ...
    //   })
    //   .then( function( ids ) {
    //     logger.debug('checkThenCollect_:reportsModel.checkIDsToRecollect:result', ids)
    //     if ( ids.length ) {
 ...
 *  @returns {promise(object)}
 */
module.exports.loadReports = function( from_, to_, accountID, onlyOverall, keepSamples ) {
  logger.info('loadReports:start');
  var from = parseDay_( from_ );
  var to = parseDay_( to_ );
    ...
    .then( function( resp ) {
      ...
      reportsList.forEach( function( acc ) {
        ...
        acc.domains_usage.forEach( function( d ) {
          du[d.name] = _.pick( d, ['traffic_per_billing_zone', 'port_hits', 'cache_hits', 'image_engine'] );
        });
      ...
      logger.info('loadReports:success finished');
      return summaries;
    })
    .catch( function( err ) {
        ...
        rep.domains_usage.forEach( function( du ) {
          ...
          _.forEach( du.traffic_per_billing_zone, function( zone ) {
            _.forEach( zone.billing_samples, function( sample, time ) {
              ...
              if (imgEngine) {
                histo[t].image_engine.optimized_image_size_bytes += imgEngine.optimized_image_size_bytes;
                histo[t].image_engine.original_image_size_bytes += imgEngine.original_image_size_bytes;
                histo[t].image_engine.optimized_images_count += imgEngine.optimized_images_count;
                imgEngine = null;
              }              
              ++samples;
            });
 ...
 *  // NOTE: used API 'Get Usage Date Histogram for an Account(s)'
 */
...
/**
 * @name  getListActiveAccountsForReports
 * @description
 *
 * @return {Array}
 */
module.exports.getListActiveAccountsForReports = function(config) {
  logger.info('getListActiveAccountsForReports:start');
  var filters = {
    subscription_id: config.subscriptionId,
    account_id: config.accountId
  };
  var day = parseDay_(config.date || 'now');
  return accountsModel.allHalfDeadIDs(day)
  // return accounts.listSubscribersAsync(filters)
    .then(function(data) {
      logger.info('getListActiveAccountsForReports:success get actuals Accounts.' +
        ' Count = ' + data.length);
      return data;
    }).catch(function() {
      logger.info('getListActiveAccountsForReports:fail retrive actual accounts');
      return [];
    });
};
//  ----------------------------------------------------------------------------------------------//
