...
var accumulateTPBZ_ = function( dest, src, skip_samples ) {
  var fields = ['count', 'received_bytes', 'sent_bytes'];
  if ( !skip_samples ) {
    fields.push( 'billing_samples' );
  }
  var totals = {
    count: 0,
    received_bytes: 0,
  ...
  for ( var zone in src ) {
    if ( dest[zone] ) {
      ...
      dest[zone] = _.pick( src[zone], fields );
    }
...
var count95BZ_ = function( tpbz, period_days ) {
  for ( var z in tpbz ) {
    var zone = tpbz[z];
    var c95p = count95p_( zone, period_days );
...
var sumAccounts_ = function( accounts, extended ) {
  ...
  accounts.forEach( function( acc ) {
    for ( var dmn in acc.domains_usage ) {
      accumulate_( res,
        accumulateTPBZ_( res.traffic_per_billing_zone, acc.domains_usage[dmn].traffic_per_billing_zone, !extended/*skip samples*/ ) );
      accumulate_( res.port_hits, acc.domains_usage[dmn].port_hits );
      accumulate_( res.cache_hits, acc.domains_usage[dmn].cache_hits );
    }
...
module.exports.loadReports = function( from, to, aids, extended, count95p ) {
  ...
  return reports.list( from, to, aids )
    .then( function( reports ) {
      var s = sumAccounts_( reports, extended );
      if ( count95p ) {
