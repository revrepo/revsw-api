...
module.exports.collectDayReport = function( day, accountID, dry, orphans ) {
  ...
  return promise.resolve( day )
    ...
    .then( function( responses ) {
      ...
      //  unite 2 sets - domains data and ssl_enabled counts
      _.forEach( accDomains.account_domains_count, function( item, aid ) {
        item.ssl_enabled = accSSLEnabled[aid] || 0;
      });
      _.forEach( accSSLEnabled, function( count, aid ) {
        if ( !accDomains.account_domains_count[aid] ) {
          accDomains.account_domains_count[aid] = {
            total: count,
            deleted: 0,
            active: count,
            ssl_enabled: count
