    ...
    .then( function( resp ) {
      ...
      if ( resp.aggregations && resp.aggregations.domains.buckets.length ) {
        traff = resp.aggregations.domains.buckets.map( function( domain ) {
          ...
          /*debug*/hosts += domain.hosts.buckets.length;

          d_.zones = domain.hosts.buckets.map( function( zone ) {
            d_.incoming_bytes += zone.total_received.value;
            d_.outgoing_bytes += zone.total_sent.value;

            /*debug*/samples += zone.samples.buckets.length;

            var billing_samples = {};
            zone.samples.buckets.forEach( function( ival ) {
              billing_samples[ival.key] = {
                count: ival.doc_count,
                incoming_bytes: ival.received.value,
                outgoing_bytes: ival.sent.value
              };
            });
            ...
            return {
              ...
              incoming_bytes: zone.total_received.value,
              outgoing_bytes: zone.total_sent.value,
...
var accumulateSamples_ = function( dest, src ) {
  for ( var key in src ) {
    if ( dest[key] ) {
      ...
      dest[key].incoming_bytes += src[key].incoming_bytes;
      dest[key].outgoing_bytes += src[key].outgoing_bytes;
    } else {
      dest[key] = src[key];
    }
  }
      ...
      responses[2/*web traffic*/].forEach( function( d ) {
        ...
        if ( !acc.domains_usage[domain] ) {
          acc.domains_usage[domain] = {
            traffic_total: {
              incoming_bytes: d.incoming_bytes,
              outgoing_bytes: d.outgoing_bytes,
              count: d.count
            },
            traffic_per_billing_zone: {},
            ports: d.ports,
        ...
        d.zones.map( function( z ) {
          ...
          if ( !domain_tpbz[zone] ) {
            ...
            domain_tpbz[zone].incoming_bytes += z.incoming_bytes;
            domain_tpbz[zone].outgoing_bytes += z.outgoing_bytes;
          ...
          if ( !account_tpbz[zone] ) {
            ...
            account_tpbz[zone].incoming_bytes += z.incoming_bytes;
            account_tpbz[zone].outgoing_bytes += z.outgoing_bytes;
