...
var count95p_ = function( billing_samples, period_days ) {
  ...
  if ( _.size( billing_samples ) <= q5 ) {
    logger.warn( 'usageReport.js:count95p_, samples amount ' + _.size( billing_samples ) + ' is less or equal to p95 threshold, too few' );
    return res;
  }
 ...
 */
module.exports.collectDayReport = function( day, account_id, dry, orphans ) {
  ...
  return promise.resolve( day )
    ...
    .then( function( responses ) {
      ...
      for ( acc_id in accDomains.account_domains_count ) {
        if ( !accounts[acc_id] ) {
          accounts[acc_id] = genEmptyAccount_( day, now, acc_id );
        }
        accounts[acc_id].domains = accDomains.account_domains_count[acc_id];
      }
      ...
      responses[0/*web traffic*/].forEach( function( domain_data ) {
        ...
        acc_id = accDomains.domain_to_acc_id_map[domain];
        if ( !acc_id ) {
