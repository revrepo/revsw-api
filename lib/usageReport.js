...
var count95p_ = function( billing_samples, period_days ) {
  ...
  //  debug
  // var test_ = Number.MAX_VALUE;
  // samples.forEach( function( s ) {
  //   if ( s.received_bps > test_ ) {
  //     throw new Error('samples not sorted by received_bps: ' + test_ + ' < ' + s.received_bps );
  //   }
  //   test_ = s.received_bps;
  // });
  // console.log( 'samples ok, max ' + samples[0].received_bps + ', p95 ' + samples[q5].received_bps + ', size ' + samples.length + ', q5 ' + q5 );
  //  debug
  ...
  //  debug
  // test_ = Number.MAX_VALUE;
  // samples.forEach( function( s ) {
  //   if ( s.sent_bps > test_ ) {
  //     throw new Error('samples not sorted by sent_bps: ' + test_ + ' < ' + s.sent_bps );
  //   }
  //   test_ = s.sent_bps;
  // });
  // console.log( 'samples ok, max ' + samples[0].sent_bps + ', p95 ' + samples[q5].sent_bps + ', size ' + samples.length + ', q5 ' + q5 );
  //  debug
...
var sumOverall_ = function( records, accounts, days, keep_samples ) {
  ...
  if ( overall ) {
    ...
    for ( id in accounts ) {
      ...
      // var found = summaries.find( function( s ) {
      //   return s.account_id === id + '_SUMMARY';
      // });
      var found = false;
      for ( var i = 0, len = summaries.length; i < len; ++i ) {
        if ( summaries[i].account_id === id + '_SUMMARY' ) {
          found = summaries[i];
          break;
        }
      }
      ...
      if ( found ) {
        overall.accounts.push( _.extend({ acc_id: id, acc_name: accounts[id] }, found.traffic ) );
      } else {
  ...
  return promise.resolve( day )
    ...
    .then( function( d ) {
      ...
      return promise.all([
        ...
        domainsModel.accountDomainsData(),
        locationsModel.code2BillingZoneMapping(),
        applicationsModel.accountAppsData(),
        apiKeysModel.accountAPIKeysData(),
    ...
    .then( function( responses ) {
      ...
      responses[0/*web traffic*/].forEach( function( domain_data ) {
        ...
        if ( !acc_id ) {
          logger.error( 'account not found for domain ' + domain_data.domain + ', skipping' );
          return;
        }
...
//  accountIds can be either ID(string), an array of IDs or an empty string for all accounts to be collected (revadmin)
//  only_overall - return overall summary only
//  keep_samples - return data with 5min samples
module.exports.loadReports = function( from, to, accountIds, only_overall, keep_samples ) {

  from = parseDay_( from );
  to = parseDay_( to );

  return promise.all([
      reportsModel.list( from, to, accountIds ),
    ...
    .then( function( resp ) {
      ...
      //  filter accounts in line with accountIds
      if ( accountIds !== '' ) {

        if ( _.isString( accountIds ) ) {
        ...
        accountIds.forEach( function( id ) {
          if ( accounts[id] ) {
            accounts_[id] = accounts[id];
          } else {
            accounts_[id] = 'Unknown';
...
//  and then check if last report too oled and then run collectDayReport before loadReports
module.exports.checkLoadReports = function( from, to, accountIds, only_overall, keep_samples ) {
  ...
  if ( to_ < today ) {
    ...
    return module.exports.loadReports( from, to, accountIds, only_overall, keep_samples );
  } else {

    return reportsModel.lastCreated(/*today*/)
      .then( function( d ) {
        if ( !d || ( Date.now() - d.getTime() ) > config.usage_report_regen_age_ms ) {
          //  today's report is absent or too old
          var refresh = {};
          return module.exports.collectDayReport( today )
...
//  ----------------------------------------------------------------------------------------------//
// debug DO NOT REMOVE
// module.exports.collectTraff = function( day ) {
