...
//  [ignore] optional param can be string with property name to skip, or object of the form { propname: 1 }
var accumulate_ = function( dest, src, ignore ) {
  for ( var key in src ) {
    if ( ignore && ( key === ignore || ignore[key] ) ) {
      continue;
    }
...
//  ---------------------------------
//  dest/src - traffic_per_billing_zone, returns total traffic counts
// var accumulateTPBZ_ = function( dest, src, skip_samples ) {

//   var totals = {};
//   skip_samples = skip_samples ? 'billing_samples' : false;
//   accumulate_( totals, src, skip_samples );
//   accumulate_( dest, src, skip_samples );
//   return totals;
// };
...
var sumAccounts_ = function( records, account_id ) {
  ...
  for ( var t in res.traffic_per_billing_zone ) {
    accumulate_( res.traffic, res.traffic_per_billing_zone[t] );
  }
