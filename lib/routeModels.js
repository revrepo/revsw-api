...
exports.APIKeyStatusModel = Joi.object({
  ...
  error: Joi.string().description('Optional description of statusCode'),
  message: Joi.string().required().description('Status message'),
  key: Joi.string().required().description('Required key for newly created API key'),
}).meta({
...
exports.listOfAPIKeysModel = Joi.array().items({
  ...
  companyId       : Joi.string().description('The ID of customer account the API key belongs to'),
  domains         : Joi.array().items(Joi.string()).description('Array of domain IDs the key should have access to'),
  createdBy       : Joi.string().description('User who created the account'),
