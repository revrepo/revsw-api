            ...
            me.getPermissions(item).then(function (permissions) {
                if (permissions) {
                    ...
                    if (item.role !== 'revadmin') {
                        ...
                        // all done, resolve the promise with the user/key
                        item.vendor_profile = defaultSystemVendorProfile;
                        item.permissions = permissions;
                        return resolve(item);
                    }
...
module.exports.checkPermissionsToResource = function (request, object, type) {
    ...
    switch (type) {
        case 'permission_list':
            ...
            // if cant pull account_id from object, take the ID.
            if (!accountId) {
                accountId = object.id;
            }
            // if accounts match, allow access to resource
        ...
        case 'waf_rules':
            // builtin rules are available to all
...
module.exports.getResellerAccs = function () {
    // returns reseller's child accounts (also set in request.auth.credentials.child_accounts)
    return childAccs;
};

module.exports.setChildAccs = function (item) {
    return new Promise(function (resolve, reject) {
        // if we are a reseller, we attach all our child accounts to our user object
        // so it can be accessible from request.auth.credentials.child_accounts
...
/**
 * Checks if a user/APIkey has access to a specific account.
 * 
 * @param {String} account id of account to check
 * @param {Object} user our user
 */
var checkAccessToAccount = function (account, user) {

    if (account === user.account_id) {
        // always allow access to a user's own account_id
        return true;
    }

    var accountMatch = false;

    // if the user has child_accounts (is a reseller), and the account is a child account
    // allow access
    ...
    if (user.permissions.accounts.access) {
        if (user.permissions.accounts.list && user.permissions.accounts.list.length > 0) {
            if (user.permissions.accounts.list.indexOf(account) !== -1) {
                /**
                 * if user's accounts permission list has something in it
                 * return the allow_list boolean value, which tells us if we can access that
                 * object or not
                 */
                ...
                if (user.child_accounts && user.child_accounts.toString().includes(account)) {
                    /**
                     * Return the opposite of allow_list flag
                     */
                    accountMatch = !user.permissions.accounts.allow_list;
                } else {
                    accountMatch = false;
        ...
        // if accounts permission is off, dont allow access
        accountMatch = false;
    }
    return accountMatch;
};