...
module.exports.getListAccountsForReport = function(config) {
  logger.info('getListAccountsForReport:start');
  var filters = {
  ...
  return accounts.listSubscribersAsync(filters)
    .then(function(data) {
      logger.info('getListAccountsForReport:success get listSubscribers.'+
        ' Count = ' + data.length );
      return data;
    }).catch(function() {
      logger.info('getListAccountsForReport:fail retrive listSubscribers');
      return [];
    });
};
/**
 * @name oneBillingReport
 * @description method get billing report data for one day for one account
 */
module.exports.oneBillingReport = function(day, account_id, dry) {
  logger.info('oneBillingReport:start get billing report for accountId ' + account_id);
  day = parseDay_(day);
  ...
  var getUsageReportForAccount = function(day, account_id, dry) {
    logger.info('oneBillingReport:getUsageReportForAccount:start for account_id ' + account_id);
    return new Promise(function(resolve, reject) {
      // NOTE: get only stored data
      var from = day,
        to = day,
        ...
        keepSamples = true;
      // NOTE: get only stored data
      usageReport.loadReports(from, to, accountId, onlyOverall, keepSamples )
        // TODO: check and delete call method usageReport.collectDayReport(day, account_id, dry)
        // it kill ES
        .then(function(data) {
          //  console.log(data)
          logger.info('oneBillingReport:getUsageReportForAccount:success get report data for Account ID ' + account_id);
          resolve(data);
        })
        .catch(function(err) {
          logger.error('oneBillingReport:getUsageReportForAccount:error get report data for Account ID ' + account_id + ' ' + JSON.stringify(err));
          resolve([]); // NOTE: all call API must be resolve
        });
    });
  };
  /**
   * @description method send Allocate Resource data to Chargify
   *
  ...
  var createAllocation = function(subscriptionId, componentId, allocation, day, dry) {
    return new Promise(function(resolve, reject) {
      logger.info('oneBillingReport:createAllocation: with Subscription Id ' + subscriptionId +
       ', Component Id ' + componentId + ' and the allocation ' + JSON.stringify(allocation));
      if (!dry) {
  ...
  var createUsage = function(subscriptionId, componentId, usage, day, dry) {
    return new Promise(function(resolve, reject) {
      logger.info('createUsage: with Subscription Id ' + subscriptionId +
        ', Component Id ' + componentId + ' and the usage ' + JSON.stringify(usage));
      // save
      if (!dry) {
  ...
  return Promise.resolve(day)
    ...
    .spread(function(account, reports, bp) {
      var reportDay = reports[0]; // NOTE:  usageReport.loadReports return array
      var listApiCalls = [];
      if (!bp) {
      ...
      return Promise.resolve(listApiCalls)
        ...
        .then(function resultAllApiCalls(data) {
          logger.info('Success sending data' + JSON.stringify(data));
          return data;
        })
        .catch(function(err) {
    ...
    .then(function(data) {
      logger.info('oneBillingReport:success finished');
      return data;
    })
    .catch(function(err) {
      logger.error('oneBillingReport:fail with error ' + JSON.stringify(err));
      return new Error('oneBillingReport:fail');
    });
};
