...
exports.Customer = {
   ...
   * @param cb
   */
  synceUsageResource: function(subscriptionId, component_id, usage, day, cb) {
    ...
    logger.info('createUsageResource::Calling Chargify to Create Usage resource for subscription ID ' + subscriptionId +
      ', component ID ' + component_id);
    // get all for update day //since_date=2016-01-20&until_date=2016-01-21
    var query = {
      per_page: 10000,
      since_date: moment(day).startOf('day').format('YYYY-MM-DD'),
      until_date: moment(day).startOf('day').add(1, 'day').format('YYYY-MM-DD')
    };
    ...
    request
      ...
      .end(function(err, res) {
        if (err) {
          logger.error('TEST LIST:error' + JSON.stringify(err));
          return cb(err, null);
        }

        var quantity_summ = 0;
        _.forEach(res.body, function(item) {
          var val = parseInt(item.usage.quantity);
          quantity_summ = quantity_summ + val;
        });

        if (quantity_summ !== 0) {
          // cancel all data
          var _usage = {
            quantity: -1 * quantity_summ,
            memo: day
          };

          createUsageResource(subscriptionId, component_id, _usage)
            .then(function() {
              return createUsageResource(subscriptionId, component_id, usage)
                .then(function(data) {
                  return cb(null, data);
                });
            })
