        ...
        function(err, res) {
          ...
          _.forEach(res.body, function summBillingSystemData(item) {
            if (item.usage.memo === usage.memo) {
              var val = parseFloat(item.usage.quantity);
              quantity_summ = quantity_summ + val;
            }
          });

          var old_val = parseFloat(usage.quantity);
          var new_val = parseFloat(usage.quantity) - quantity_summ;

          // TODO: Check Flaot falue !!!!
          if (new_val === 0) {
            logger.info('syncUsageResource: Data already sync');
            return cb(new Error('Data already sync'));
          }
