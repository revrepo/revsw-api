          ...
          function(err, data) {
            if (err) {
              ...
              if (!!err.status && err.status === 422) {
                ...
                reject(new Error('Billing System API call error'));
                throw new Error('Billing System API call error');
              }
            } else {
      ...
      _.forEach(bp.services, function(servise) {
        ...
        if (servise.code_name === 'App') {
          ...
          listApiCalls.push(function(dry) {
            var allocation = {
              quantity: reportDay.applications.total,
              memo: data_memo
            };
        ...
        if (servise.code_name === 'Domain') {
          listApiCalls.push(function(dry) {
            var allocation = {
              quantity: reportDay.domains.total,
              memo: data_memo
            };
            var component_id = servise.chargify_component_id;
            return createAllocation(account.subscription_id, component_id, allocation, day, dry);
        ...
        if (servise.code_name === 'request') {
          listApiCalls.push(function(dry) {
            var usage = {
              quantity: reportDay.traffic.count / 10000,
              memo: data_memo
            };
            var component_id = servise.chargify_component_id;
        ...
        if (servise.code_name === 'traffic') {
          listApiCalls.push(function(dry) {
            var val = (reportDay.traffic.sent_bytes + reportDay.traffic.received_bytes) / 1000000;
            var usage = {
              quantity: val,
              memo: data_memo
            };
