...
var getDnsZonesStatsUsageZone = function(zoneName, options) {
  logger.info('Calling NSONE API to DNS zones usage. Zone ' + zoneName + ' with options '+JSON.stringify(options));
  return new NS1.NS1Request(
    'get',
    'stats/usage/'+zoneName,
    {
      period: options.period,
      expand: false, // if true - stats will be returned for all records in the zone
      aggregate: true, // if set, returns aggregated stats across all zones and billing tiers
      by_tier: false,
    }
  );
};
/**
 * Get data Zones Usage by last 24 hours
 */
var getDnsZonesUsageReport24h = function() {
  logger.info('Calling NSONE API to DNS zones usage');
  return sendNsoneRequest(
    'GET',
    'stats/usage',
...
module.exports = {
  ...
  request: NS1.NS1Request,
  getDnsZonesStatsUsageZone: getDnsZonesStatsUsageZone,
  getDnsZonesUsageReport24h: getDnsZonesUsageReport24h
};
