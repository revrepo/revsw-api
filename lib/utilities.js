...
module.exports = {
  ...
  checkUserAccessPermissionToAPIKey : function(request, apiKey){
    return request && request.auth.credentials.role === 'revadmin' &&
            apiKey.account_id && request.auth.credentials.companyId.indexOf(apiKey.account_id) !== -1;
  },

  checkUserAccessPermissionToApps : function(request, app){
    return request && request.auth.credentials.role === 'revadmin' &&
            app.account_id && request.auth.credentials.companyId.indexOf(app.account_id) !== -1;
  },
