...
module.exports = {
  ...
  roundTimestamps : function(options,rangMin ){
    ...
    if(!!options.from_timestamp) {
      var diffFrom_ = moment(parseInt(options.from_timestamp))
        .diff(moment(parseInt(options.from_timestamp)).startOf('hours'), 'minutes');
      updatedProperties.from_timestamp = moment(parseInt(options.from_timestamp))
        .startOf('minutes')
        .subtract(diffFrom_ % rangMin || 5, 'minutes').valueOf();
    }
    if(!!options.to_timestamp) {
      ...
      updatedProperties.to_timestamp = moment(parseInt(options.to_timestamp))
        .startOf('minutes')
        .subtract(diffTo_ % rangMin || 5, 'minutes').valueOf();
    }
