...
//        console.log(res.text);
  ...
  buildIndexList : function (start_time, end_time) {
    var d_start = new Date(start_time),
      d_end = new Date(end_time);

    d_start = d_start.getUTCFullYear() + '.' + ('0' + (d_start.getUTCMonth() + 1)).slice(-2) + '.' + ('0' + d_start.getUTCDate()).slice(-2);
    d_end = d_end.getUTCFullYear() + '.' + ('0' + (d_end.getUTCMonth() + 1)).slice(-2) + '.' + ('0' + d_end.getUTCDate()).slice(-2);

    return d_start === d_end ? 'logstash-' + d_start : 'logstash-' + d_start + ',logstash-' + d_end;
  },
