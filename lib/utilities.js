...
module.exports = {
    ...
    switch (request.auth.credentials.user_type) {
      case 'user':
        if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = request.auth.credentials.permissions.ssl_names;
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(sslName.account_id) !== -1;

          return (permissionData && accountMatch) || revAdmin;
        }
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,sslName.account_id);
  ...
  checkUserAccessPermissionToLogShippingJob: function(request, job) {
    ...
    switch (request.auth.credentials.user_type) {
      ...
      if (request.auth.credentials.permissions) {
        // use new permissions system (Groups and ACL)      
        var revAdmin = request.auth.credentials.role === 'revadmin';
        var permissionData = request.auth.credentials.permissions.logshipping_jobs;
        var accountMatch = true;//request.auth.credentials.companyId.indexOf(job.account_id) !== -1;
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,job.account_id);
  ...
  checkUserAccessPermissionToSSLCertificate: function(request, cert) {
    ...
    switch (request.auth.credentials.user_type) {
      ...
      if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = request.auth.credentials.permissions.ssl_certs;
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(cert.account_id) !== -1;
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,cert.account_id);
  ...
  /**
   * @name checkUserAccessPermissionToWAFRule
   * @classdesc Check permission to WAF Rule
   */
  checkUserAccessPermissionToWAFRule: function(request, wafRule) {
    // Rules with “visibility” set to “hidden” should be visible to “revadmin” role only
    if (wafRule.rule_type === 'builtin' && wafRule.visibility === 'hidden') {
      if (request.auth.credentials.user_type === 'user' && this.isUserRevAdmin(request)) {
        return true;
      } else {
        return false;
      }
    }
    // All users have access to Public  Buil-In WAF Rule
    if (wafRule.rule_type === 'builtin' && wafRule.visibility === 'public') {
      return true;
    }
    // Rule with type equals "customer"
    if (wafRule.rule_type === 'customer') {
      switch (request.auth.credentials.user_type) {
        case 'user':
        if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = request.auth.credentials.permissions.waf_rules;
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(wafRule.account_id) !== -1;

          return (permissionData && accountMatch) || revAdmin;
        }
          if (this.isUserRevAdmin(request)  === false && request.auth.credentials.companyId.indexOf(wafRule.account_id) === -1) {
            return false;
          }
          return true;

        case 'apikey':
          return this.isHasAPIkeyAccessToAccount(request.auth.credentials,wafRule.account_id);
  ...
  /**
   * @name checkUserAccessPermissionToAccount
   *
   */
    ...
    switch (request.auth.credentials.user_type) {
      case 'user':
        ...
        if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = this.checkPermissionsToResource(request.auth.credentials.permissions.accounts, account_id);
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(account_id) !== -1;
        ...
        return false;
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,account_id);
  ...
  checkUserAccessPermissionToAPIKey : function(request, apiKey){
    ...
    switch (request.auth.credentials.user_type) {
      ...
      if (request.auth.credentials.permissions) {
        // use new permissions system (Groups and ACL)
        var revAdmin = request.auth.credentials.role === 'revadmin';
        var permissionData = request.auth.credentials.permissions.API_keys;
        var accountMatch = true;//request.auth.credentials.companyId.indexOf(apiKey.account_id) !== -1;
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials, apiKey.account_id);
    ...
    switch (request.auth.credentials.user_type) {
      case 'user':
        if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = this.checkPermissionsToResource(request.auth.credentials.permissions.mobile_apps, app);
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(app.account_id) !== -1;

          return (permissionData && accountMatch) || revAdmin;
        } else {
          // use old premissions system
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,app.account_id);
  ...
  checkUserAccessPermissionToUser: function(request, userObject) {
    ...
    switch (request.auth.credentials.user_type) {
      ...
      if (request.auth.credentials.permissions) {
        // use new permissions system (Groups and ACL)
        var revAdmin = request.auth.credentials.role === 'revadmin';
        var permissionData = request.auth.credentials.permissions.users;
        var accountMatch = true;//request.auth.credentials.companyId.indexOf(userObject.companyId[0]) !== -1;
        ...
        if( request.auth.credentials.companyId.indexOf(userObject.companyId[0]) !== -1) {
           return true;
        }
      ...
      case 'apikey':
        ...
        var myAccountIds_ = this.getAccountID(request);

        // No access to revadmin users for API keys
        if (userObject.role === 'revadmin') {
          return false;
        }

        if (myAccountIds_.indexOf(userObject.companyId[0]) !== -1) {
          return true;
        }
  ...
  // currently only rev admin can access groups
  checkUserAccessPermissionToGroup: function(request, groupObject) {
    switch (request.auth.credentials.user_type) {
      case 'user':
        if(request.auth.credentials.role === 'revadmin') {
          return true;
        ...
        var permissionData = request.auth.credentials.permissions.groups;
    ...
    switch (request.auth.credentials.user_type) {
      case 'user':
        if (request.auth.credentials.permissions) {
          // use new permissions system (Groups and ACL)      
          var revAdmin = request.auth.credentials.role === 'revadmin';
          var permissionData = this.checkPermissionsToResource(request.auth.credentials.permissions.domains, domainObject);
          var accountMatch = true;//request.auth.credentials.companyId.indexOf(domainObject.account_id) !== -1;
        ...
        return true;
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,accountId);
  ...
  checkUserAccessPermissionToDashboard: function(request, dashboardObject) {
    if (request.auth.credentials.permissions) {
      // use new permissions system (Groups and ACL)      
      var permissionData = request.auth.credentials.permissions.dashboards;
  ...
  checkUserAccessPermissionToDNSZone: function(request, dnsZoneObject) {
    switch (request.auth.credentials.user_type) {
      ...
      if (request.auth.credentials.permissions) {
        // use new permissions system (Groups and ACL)      
        var revAdmin = request.auth.credentials.role === 'revadmin';
        var permissionData = this.checkPermissionsToResource(request.auth.credentials.permissions.dns_zones, dnsZoneObject);
        var accountMatch = true;//request.auth.credentials.companyId.indexOf(dnsZoneObject.account_id) !== -1;
      ...
      case 'apikey':
        return this.isHasAPIkeyAccessToAccount(request.auth.credentials,dnsZoneObject.account_id);
    ...
    switch(request.auth.credentials.user_type){
      case 'user':
        if(isMainAccountIdOnly === true){
          ...
          return request.auth.credentials.companyId[0];
        }
  ...
  convertFirstCharacterToUpperCase: function(text) {
    ...
    /**
   * @name checkPermissionsToResource
   * @description This method checks if access is allowed to a specific item
   */
  checkPermissionsToResource: function(list, item) {
    // get our permissions for needed resource
    var accessList = list;

    // if the global access flag is false, don't authorize!
    ...
    if (accessList.list && accessList.length !== 0) {
      ...
      if (accessList.list.indexOf(item.id ? item.id.toString() : item) !== -1) {
        // if the resource is on the access list
        return accessList.allow_list; // return either allow access to this resource or not
      } else {
        // return the oposite!
        return !accessList.allow_list;
      }
    }
