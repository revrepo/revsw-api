...
module.exports = {
  ...
  /**
   * creates time span from the given query and parameters and not round time
   *
   * @param {Object} query
   * @param {int} default value of start in hours back from now
   * @param {int} maximum allowed period in hours
   */
  query2SpanNoRound: function( query, hrs_back, hrs_allowed ) {
    ...
    if ( span.start >= span.end ) {
      // 'The period end timestamp cannot be less or equal of the period start timestamp'
      span.error = 'Period end timestamp cannot be less or equal period start timestamp';
      return span;
    }
    if ( ( span.end - span.start ) > ( hrs_allowed * 3600000 + 10000/*loose it a little*/ ) ) {
