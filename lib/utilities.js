    ...
    if (isMainAccountIdOnly) {
      ...
      if (user.child_accounts && user.child_accounts.length > 0 && user.role === 'reseller') {
        if (permissions.accounts.list && permissions.accounts.list.length > 0) {
          for (var i = 0; i < permissions.accounts.list.length; i++) {
            var acc = permissions.accounts.list[i];
            for (var j = 0; j < user.child_accounts.length; j++) {
              var childAcc = user.child_accounts[j];
              if (permissions.accounts.allow_list && (acc === childAcc)) {
                returnAccounts.push(childAcc);
              } else if (permissions.accounts.list.indexOf(childAcc) === -1) {
                returnAccounts.push(childAcc);
              }
            }
          }
          ...
          return returnAccounts.concat(user.child_accounts);
        }
      } else {
        return returnAccounts;
      }
