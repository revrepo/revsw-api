    ...
    switch (request.auth.credentials.user_type) {
      case 'user':
        if (request.auth.credentials.permissions) {
          ...
          // get our mobile access data
          var mobileAppsAccess = request.auth.credentials.permissions.mobile_apps;

          // rev admin always has access to everything
          if (request.auth.credentials.role === 'revadmin') {
            return true;
          }

          // if the global access flag to mobile apps is false, don't authorize!
          if (!mobileAppsAccess.access) {
            return false;
          } else if (mobileAppsAccess.access && (!mobileAppsAccess.apps || mobileAppsAccess.apps.length === 0)) {
            if (request.auth.credentials.companyId.indexOf(app.account_id) !== -1) {
              // the global access flag is true, and there are no apps specified, grant full access
              return true;
            }
          }

          // check if there are specific apps listed
          if (mobileAppsAccess.apps && mobileAppsAccess.length !== 0 && request.auth.credentials.companyId.indexOf(app.account_id) !== -1) {
            // if the account matches the app account
            if (mobileAppsAccess.apps.indexOf(app.id) !== -1) {
              // if the app is on the access list
              return mobileAppsAccess.allow_list; // return either allow access to this app or not
            } else {
              // return the oposite!
              return !mobileAppsAccess.allow_list;
            }
          }

          // return false if something is not right
          return false;
        } else {
