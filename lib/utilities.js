...
//        console.log(res.text);
  ...
  buildIndexList : function (start_time, end_time) {
    var index = '';
    for (var i = start_time; i < ( end_time + 24 * 3600 * 1000); i = i + 24 * 3600 * 1000) {
      var d = new Date(i);
      index = (index === ''  ) ? index : index + ',';
      //  UTC - for some stupid case when server not in UTC tz
      index = index + 'logstash-' + d.getUTCFullYear() + '.' + ('0' + (d.getUTCMonth() + 1)).slice(-2) + '.' + ('0' + d.getUTCDate()).slice(-2);
    }
    return index;
  },
