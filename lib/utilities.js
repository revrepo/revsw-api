...
module.exports = {
  ...
  checkUserAccessPermissionToDomain: function(request, domainObject) {
    if (request.auth.credentials.role === 'revadmin' || (request.auth.credentials.companyId.indexOf(domainObject.companyId) !== -1 &&
      request.auth.credentials.domain.indexOf(domainObject.name) !== -1)) {
