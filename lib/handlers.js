...
exports.getDetailedAuditInfo = function (request, reply) {
  request.query.user_id = request.auth.credentials.user_id;
  auditevents.detailed(request.query, function (error, result) {
    renderJSON(request, reply, error, result);
  });
};
...
exports.getSummaryAuditInfo = function (request, reply) {
  ...
  for (var key in request.query) {
    filter['meta.'+key] = request.query[key];
  }
  filter['meta.user_id'] = request.auth.credentials.user_id;
  console.log(filter);
  auditevents.summary(filter, function (error, result) {
    renderJSON(request, reply, error, result);
  });
};
