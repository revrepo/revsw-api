...
function purgeObject(request, reply) {
  ...
  if ( request.auth.credentials.role !== 'admin' && request.auth.credentials.role !== 'reseller' ) {
    return reply( boom.forbidden('Not enough privileges to purge cached objects') );
  }
...
exports.createUser = function (request, reply) {
  if ( request.auth.credentials.role !== 'admin' && request.auth.credentials.role !== 'reseller' ) {
    return reply( boom.forbidden('Not enough privileges to create new user') );
  }
...
exports.getUser = function (request, reply) {
  if ( request.auth.credentials.role === 'user' ) {
    return reply( boom.forbidden('Not enough privileges to access user details') );
  }
...
exports.deleteUser = function (request, reply) {
  if ( request.auth.credentials.role === 'user' ) {
    return reply( boom.forbidden('Not enough privileges to delete a user') );
  }
  var user_id = request.params.user_id;
