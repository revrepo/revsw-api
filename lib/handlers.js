...
var UserConnection = mongoose.createConnection(Config.portal_mongo.connect_string),
  users = new User(mongoose, UserConnection);

UserConnection.on('error', function(err) {
  console.log(['error'], 'moungodb connect error: ' + err);
});

UserConnection.on('connected', function() {  
