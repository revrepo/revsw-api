  ...
  domains.get({
    ...
    if (result && request.auth.credentials.companyId.indexOf(result.companyId) !== -1 && request.auth.credentials.domain.indexOf(result.name) !== -1) {
      ...
      switch (request.query.report_type) {
        case 'cache_miss':
          filter = ' AND (cache: \'MISS\' OR cache: \'-\')';
          break;
        case 'cache_hit':
          filter = ' AND cache: \'HIT\'';
          break;
        case 'status_code':
          if ( ! request.query.status_code) {
            return reply(boom.badRequest('"status_code" query parameter must be specified for "status_code" reports'));
          }
          filter = ' AND response: \'' + request.query.status_code + '\'';
          break;
        default:
          filter = '';
      }
...
//
// Handler for Top Referers report
//

exports.getTopReferers = function(request, reply) {
