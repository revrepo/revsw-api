  ...
  var scheme = {
    ...
    authenticate: function (request, reply) {
      ...
      if (!request.headers['x-chargify-webhook-id']) {
        logger.warn('Auth Scheme: \'signature\'::authenticate:  Chargify webhook ID is missing from headers');
      ...
      if (!request.headers['x-chargify-webhook-signature-hmac-sha-256']) {
        logger.warn('Auth Scheme: \'signature\'::authenticate:  Chargify webhook signature is missing from headers');
    ...
    payload: function (request, reply) {
      ...
      if(hmacHex === signature) {
        ...
        logger.warn('Auth Scheme: \'signature\'::payload: Signature mismatch. hmacHex = ' + hmacHex);
