/*************************************************************************
 *
 * REV SOFTWARE CONFIDENTIAL
 *
 * [2013] - [2017] Rev Software, Inc.
 * All Rights Reserved.
 *
 * NOTICE:  All information contained herein is, and remains
 * the property of Rev Software, Inc. and its suppliers,
 * if any.  The intellectual and technical concepts contained
 * herein are proprietary to Rev Software, Inc.
 * and its suppliers and may be covered by U.S. and Foreign Patents,
 * patents in process, and are protected by trade secret or copyright law.
 * Dissemination of this information or reproduction of this material
 * is strictly forbidden unless prior written permission is obtained
 * from Rev Software, Inc.
 */

/*jslint node: true */

'use strict';

var utils = require('./utilities');

var publicRecordFields = {

  accounts: [
    'id',
    'createdBy',
    'companyName',
    'vendor_profile',
    'comment',
    'billing_plan',
    'subscription_state',
    'created_at',
    'updated_at',
    'parent_account_id'
  ],

  account: [
    'id',
    'createdBy',
    'companyName',
    'vendor_profile',
    'comment',
    'country',
    'state',
    'city',
    'address1',
    'address2',
    'zipcode',
    'phone_number',
    'first_name',
    'last_name',
    'contact_email',
    'created_at',
    'updated_at',
    'billing_id',
    'billing_plan',
    'billing_info',
    'billing_portal_link',
    'use_contact_info_as_billing_info',
    'subscription_state',
    'subscription_id',
    'self_registered',
    'valid_payment_method_configured',
    'parent_account_id'
  ],


  apiKeys: [
    'id',
    'key',
    'key_name',
    'account_id',
    'created_by',
    'active',
    'created_at',
    'updated_at',
    'permissions',
    'group_id',
    'role',
    'last_used_at',
    'last_used_from'
  ],


  apiKey: [
    'id',
    'key',
    'key_name',
    'account_id',
    'created_by',
    'active',
    'created_at',
    'updated_at',
    'permissions',
    'group_id',
    'role',
    'last_used_at',
    'last_used_from'
  ],

  apps: [
    'id',
    'app_name',
    'comment',
    'account_id',
    'app_platform',
    'sdk_key',
    'created_at',
    'created_by',
    'updated_at',
    'updated_by',
    'configs'
  ],

  domains: [
    'id',
    'name',
    'cname',
    'companyId',
    'btt_key',
    'sync_status'
  ],

  domain: [
    'id',
    'name',
    'companyId',
    'sync_status',
    'cname',
    'comment',
    'origin_server',
    'btt_key',
    'tolerance',
    'origin_server_location',
    'origin_host_header',
    'created_at',
    'updated_at',
    'updated_by'
  ],

  masterConfiguration: [
    'enable_origin_health_probe',
    'origin_health_probe',
    'rev_component_co',
    'rev_component_bp'
  ],

  masterConfigurationBp: [
    'enable_cache',
    'block_crawlers',
    'cdn_overlay_urls',
    'caching_rules',
    'enable_security',
    'web_app_firewall',
    'origin_health_probe',
    'acl',
    'cache_bypass_locations'
  ],

  /*masterConfigurationCo : [
    'ssl_certificates',
    'certificate_urls',
    'bp_apache_custom_config',
    'bp_apache_fe_custom_config',
    'cache_opt_choice'
  ]*/
  purge: [
    'req_id',
    'req_domain',
    'req_email',
    'request_json'
  ],

  verify: [
    'firstname',
    'lastname',
    'email',
    'address1',
    'address2',
    'country',
    'state',
    'zipcode',
    'phone_number',
    'city',
    'companyName',
    'token'
  ],

  users: [
    'user_id',
    'email',
    'firstname',
    'lastname',
    'role',
    'two_factor_auth_enabled',
    'updated_at',
    'last_login_at',
    'group_id',
    'account_id'
  ],

  user: [
    'user_id',
    'email',
    'firstname',
    'lastname',
    'role',
    'theme',
    'created_at',
    'updated_at',
    'two_factor_auth_enabled',
    'last_login_at',
    'last_login_from',
    'comment',
    'invitation_token',
    'invitation_expire_at',
    'invitation_sent_at',
    'group_id',
    'permissions',
    'account_id'
    //    'self_registered'
  ],

  groups: [
    'id',
    'name',
    'comment',
    'account_id',
    'updated_at',
    'updated_by'
  ],

  group: [
    'id',
    'name',
    'comment',
    'account_id',
    'permissions',
    'created_at',
    'updated_at',
    'created_by',
    'updated_by'
  ],

  groupUsers: [
    'count',
    'users'
  ],

  billingPlans: [
    'id',
    'name',
    'description',
    'chargify_handle',
    'hosted_page',
    'type',
    'monthly_fee',
    'services',
    'prepay_discounts',
    'commitment_discounts',
    'order',
    'created_at',
    'updated_at',
    'brand'
  ],

  billingPlan: [
    'id',
    'name',
    'description',
    'chargify_handle',
    'hosted_page',
    'type',
    'monthly_fee',
    'services',
    'prepay_discounts',
    'commitment_discounts',
    'order',
    'deleted',
    'history',
    'overage_credit_limit',
    'created_at',
    'updated_at',
    'brand'
  ],

  statements: [
    'id',
    'total_in_cents',
    'html_view',
    'opened_at',
    'settled_at',
    'created_at',
    'closed_at',
    'memo',
    'success'
  ],

  statement: [
    'id',
    'basic_html_view',
    'closed_at',
    'created_at',
    'customer_billing_address',
    'customer_billing_address_2',
    'customer_billing_city',
    'customer_billing_country',
    'customer_billing_state',
    'customer_billing_zip',
    'customer_first_name',
    'customer_last_name',
    'customer_organization',
    'ending_balance_in_cents',
    'payments',
    'future_payments',
    'html_view',
    'memo',
    'opened_at',
    'settled_at',
    'starting_balance_in_cents',
    'subscription_id',
    'text_view',
    'updated_at',
    'total_in_cents',
    'transactions',
    'events',
    'payments_total'
  ],

  transactions: [
    'id',
    'transaction_type',
    'amount_in_cents',
    'starting_balance_in_cents',
    'ending_balance_in_cents',
    'created_at',
    'closed_at',
    'memo',
    'success'
  ],

  dashboards: [
    'id',
    'title',
    'created_at',
    'updated_at'
  ],

  dashboard: [
    'id',
    'user_id',
    'title',
    'structure',
    'options',
    'rows',
    'created_at',
    'updated_at'
  ],

  sslCertificates: [
    'id',
    'account_id',
    'cert_type',
    'cert_name',
    'created_by',
    'comment',
    'created_at',
    'updated_at',
    'expires_at',
    'domains'
  ],

  sslCertificate: [
    'id',
    'account_id',
    'cert_type',
    'cert_name',
    'created_by',
    'comment',
    'created_at',
    'updated_at',
    'updated_by',
    'last_published_ssl_config_version',
    'public_ssl_cert',
    'private_ssl_key',
    'private_ssl_key_passphrase',
    'chain_ssl_cert',
    'expires_at',
    'domains'
  ],

  sslConfigurationProfiles: [
    'id',
    'profile_name',
    'ssl_protocols',
    'ssl_ciphers',
    'ssl_prefer_server_ciphers',
    'comment'
  ],

  wafRules: [
    'id',
    'account_id',
    'rule_name',
    'rule_type',
    'visibility',
    'created_by',
    'comment',
    'created_at',
    'updated_at',
    'updated_by',
    'domains'
  ],

  wafRule: [
    'id',
    'account_id',
    'rule_name',
    'rule_type',
    'rule_body',
    'visibility',
    'created_by',
    'comment',
    'created_at',
    'updated_at',
    'updated_by',
    'domains',
    'operation'
  ],

  logShippingJobs: [
    'id',
    'job_name',
    'operational_mode',
    'account_id',
    'created_by',
    'created_at',
    'source_type',
    'source_id',
    'destination_type',
    'destination_host',
    'comment',
    'updated_by',
    'updated_at'
  ],

  logShippingJob: [
    'id',
    'job_name',
    'operational_mode',
    'account_id',
    'created_by',
    'created_at',
    'source_type',
    'source_id',
    'destination_type',
    'destination_host',
    'destination_port',
    'destination_key',
    'destination_username',
    'destination_password',
    'notification_email',
    'comment',
    'updated_by',
    'updated_at'
  ],

  sslNames: [
    'id',
    'ssl_name',
    'account_id',
    'created_by',
    'created_at',
    'updated_by',
    'updated_at',
    'verification_method',
    'verification_object',
    'verified',
    'published',
    'deployment_status'
  ],

  sslName: [
    'id',
    'ssl_name',
    'account_id',
    'created_by',
    'created_at',
    'updated_by',
    'updated_at',
    'verification_method',
    'verification_object',
    'verified',
    'published',
    'deployment_status'
  ],

  vendorProfiles: [
    'vendor',
    'vendorUrl',
    'sign_up_url',
    'support_email',
    'companyNameShort',
    'companyNameFull',
    'companyWebsiteURL',
    'contactSalesLink',
    'contactUsLink',
    'knowledgeBaseLink',
    'apiUrl',
    'contactPhone',
    'networkStatusUrl',
    'domain_quick_start_guide_url',
    'sdk',
    'terms_of_service_link',
    'acceptable_use_policy_link',
    'privacy_policy_link',
    'zendesk_support_site',
    'copyright',
    'googleAnalyticsAccount',
    'swaggerUIPageTitle',
    'swaggerUICDNCustomerAPIUrl',
    'apiGoogleAnalyticsAccount',
    'enable_simplified_signup_process'
  ],
  notificationLists:[
    'id',
    'account_id',
    'list_name',
    'created_by',
    'created_at',
    'updated_by',
    'updated_at'
  ],

  trafficAlerts:[
    'id',
    'name',
    'account_id',
    'target_type',
    'target',
    'rule_type',
    'rule_config',
    'status',
    'created_at',
    'created_by',
    'updated_at',
    'updated_by',
    'notifications_list_id',
    'silenced'
  ],

  notificationList: [
    'id',
    'account_id',
    'list_name',
    'destinations',
    'created_by',
    'created_at',
    'updated_by',
    'updated_at'
  ]
};

/**
 *
 * @param data
 * @param allowFields
 * @returns {*}
 */
var handler = function(data, allowFields) {

  var result;

  if (utils.isArray(data)) {

    result = [];

    for (var i in data) {
      var obj = {};
      for (var j in allowFields) {
        if (allowFields[j] in data[i]) {
          obj[allowFields[j]] = data[i][allowFields[j]];
        }
      }
      result.push(obj);
    }

  } else {

    result = {};

    for (var o in allowFields) {
      if (allowFields[o] in data) {
        result[allowFields[o]] = data[allowFields[o]];
      }
    }

  }

  return result;
};

/**
 *
 * @param {Object} data
 * @param {String} action
 * @returns {{Object|Error}}
 */
exports.handle = function(data, action) {
  if (publicRecordFields[action]) {
    return handler(data, publicRecordFields[action]);
  } else {
    return new Error('No-existing type of record "'+action +'" is specified as parameter to function publicRecordFields');
  }
};
