...
var updateByRequest_ = function( subquery, request ) {
  if (request.query.cache_code) {
    var isSecondaryCache = request.query.cache_type ? (request.query.cache_type === 'secondary') : false;
    if(isSecondaryCache === true){
      subquery.must.push({ exists: { field: 'cache2' } });
      subquery.must_not.push({ term: { cache2: '-' } });
      if(request.query.cache_code === 'MISS') {
        subquery.must.push({ terms: { cache: ['MISS', '-'] } });
        subquery.must.push({ terms: { cache2: ['MISS'] } });
      } else {
        subquery.must.push({ term: { cache: 'MISS' } });
        subquery.must.push({ term: { cache2: 'HIT' } });
      }
    } else {
      if ( request.query.cache_code === 'MISS' ) {
        subquery.must.push({ terms: { cache: ['MISS','-'] } });
  ...
  if (request.query.status_code !== undefined) {
...
/**
 *  @name buildESQueryTermsForNaxsi
 *  @description method for update filter ES query for NAXSI data
 *
 *  @param {object} - part of query to push result into, assuming `query: { filtered: { filter: { bool: {} } } } ...`
 *  @param {object} - hapi request object to get parameters from, or false
 *  @param {object} - domains config object to get parameters from, or false
 */
exports.buildESQueryTermsForNaxsi = function(subquery, request, domainConfig) {
