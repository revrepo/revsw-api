  ...
  this.resolve(name, 'CNAME', function(err, answer) {
    if (!err) {
      if (utils.isArray(answer.data) && answer.data.length !== 0) {
        if (answer.data.indexOf(cname) !== -1) { // TODO Please double-check the code - it does not 
          // recognize if a domain is pointing to a CNAME but it is not the expected CNAME
