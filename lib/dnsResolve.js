...
exports.resolve = function(name, type_, cb) {
  ...
  standardDNS.resolve(name, type_, function(err, answer) {
    if(answer === undefined) {
      // undefined answer === domain not found in root dns system
      resolveInfo.message = 'Could not get "' + type_ + '" record for "' + name + '"';
      resolveInfo.error = 'The specified domain name is not found in the global DNS system';
      return cb(resolveInfo);
    }
  ...
  return new Promise(function(resolve, reject) {
    ...
    req.on('message', function(err, answer) {
      ...
      if (!!answer && !!answer.answer) {
        var questionForDomainName_ = answer.question[0].name;
        answer.answer = _.filter(answer.answer, function(itemAnswer) {
          // NOTE: answer.answer contain information about all found records linked with cheking domain
          return (nativeDNSPacketConsts.nameToQtype(questionOption.type) === itemAnswer.type) &&
           (questionForDomainName_ === itemAnswer.name);
        });
      }
      resolve(answer);
    });
    ...
    req.on('timeout', function(err) {
      logger.error('dnsRessolve::getDNSData:timeout event for server: ' + serverAddres + ' and question ' + JSON.stringify(questionOption));
      reject(err);
    });
 ...
 * NOTE: one record type - one "Case <typeName>:". Don't use "Case <typeName_1>, <typeName_2>:"
 *
 * @param {String} type_
 * @param {Object} dnsAnswer_ - an answer object (see npm package 'native-dns')
 * @return {Array}
 */
exports.formatToNSONEAnswerFromFullInfo = function formatToNSONEAnswerFromFullInfo(type_, dnsAnswer_) {
  ...
  switch (type) {
    ...
    case 'SRV':
      ...
      return new Array(dnsAnswer.priority, dnsAnswer.weight, dnsAnswer.port, dnsAnswer.target);
    case 'NS':
