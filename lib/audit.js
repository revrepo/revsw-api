...
//  }
  ...
  if(request){
    object.ip_address = utils.getAPIUserRealIP(request);
    object.user_id = request.auth.credentials.user_id || request.auth.credentials.id;
    object.user_name = request.auth.credentials.email;
    object.user_type = object.user_type || request.auth.credentials.user_type;
    object.target_id = object.target_id || request.params.account_id;
    object.account_id = object.account_id || (object.user_type === 'user' ? request.auth.credentials.companyId[0] : request.auth.credentials.account_id);
  }
