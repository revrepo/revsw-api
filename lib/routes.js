  ...
  config: {
    ...
    validate: {
      ...
      query: {
        ...
        report_type: Joi.string().valid ( 'requests', 'cache_hit', 'cache_miss', 'status_code' ).description('Type of requested top object report'),
...
//    }
 ...
 {
  method: 'GET',
  path: '/v1/stats/top_referers/{domain_id}',
  config: {
    auth: {
      strategy: 'simple',
      scope: [ 'user', 'admin', 'reseller' ]
    },
    handler: handlers.getTopReferers,
    description: 'Get a list of top referers for a domain',
    tags: ['api'],
    plugins: {
      'hapi-swagger': {
        responseMessages: standardHTTPErrors
    ...
    validate: {
      ...
      query: {
        from_timestamp: Joi.number().integer().description('Report period start timestamp (defaults to one hour ago from now)'),
        to_timestamp: Joi.number().integer().description('Report period end timestamp (defaults to now)'),
        count: Joi.number().integer().description('Number of top referers to report (default to 30)'),
        country: Joi.string().description('Two-letters country code of end user location'),
      }
    },
//    response: {
