...

 {
  ...
  config: {
    ...
    validate: {
      ...
      query: {
        from_timestamp: Joi.number().integer().description('Report period start timestamp'),
        to_timestamp: Joi.number().integer().description('Report period end timestamp'),
        interval: Joi.number().integer().description('Reporting interval in seconds'),
        country: Joi.string().description('Two-letters country code of end user location'),
      }
    },
//    response: {
//      schema: statsModel
...

 {
  ...
  config: {
    ...
    validate: {
      ...
      query: {
        from_timestamp: Joi.number().integer().description('Report period start timestamp (defaults to one hour ago from now)'),
        to_timestamp: Joi.number().integer().description('Report period end timestamp (defaults to now)'),
        count: Joi.number().integer().description('Number of top objects to report (default to 30)'),
        report_type: Joi.string().valid ( 'requests', 'cache_hit', 'cache_miss', 'status_code' )
          .description('Type of requested top object report (default to "requests")'),
        status_code: Joi.number().integer().description('HTTP status code for "status_code" report'),
        country: Joi.string().description('Two-letters country code of end user location'),
      }
    },
//    response: {
  ...
  config: {
    ...
    validate: {
      ...
      query: {
        from_timestamp: Joi.number().integer().description('Report period start timestamp (defaults to one hour ago from now)'),
        to_timestamp: Joi.number().integer().description('Report period end timestamp (defaults to now)'),
        count: Joi.number().integer().description('Number of entries to report (default to 30)'),
        report_type: Joi.string().valid ( 'referer', 'status_code', 'content_type', 'protocol', 'http_protocol', 'http_method', 'content_encoding' )
          .description('Type of requested report (default to "referer")'),
        country: Joi.string().description('Two-letters country code of end user location'),
      }
    },
//    response: {
