...
var listOfUsersModel = Joi.array().items({
  ...
  domain: Joi.array().items( Joi.string().required().description('Domain name the customer can manage') ),
...

{
  ...
  config: {
    ...
    notes: 'Use the function to purge objects from Rev edge caching servers. You can specify several "url" objects to purge multiple cached objects at once. After ' +
      'a purge request is submitted to the system you should remember received "request_id" ID and use it to check the status of the request using /v1/purge/{request_id} ' +
      'GET call.',
    tags: ['api', 'purge'],
    plugins: {
      'hapi-swagger': {
