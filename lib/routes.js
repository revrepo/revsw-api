...
//    }
 ...
 {
  method: 'GET',
  path: '/v1/stats/top_requests/{domain_id}',
  config: {
    auth: {
      strategy: 'simple',
      scope: [ 'user', 'admin', 'reseller' ]
    },
    handler: handlers.getTopRequests,
    description: 'Get a list of most requested objects for a domain',
    tags: ['api'],
    plugins: {
      'hapi-swagger': {
        responseMessages: standardHTTPErrors
    ...
    validate: {
      ...
      query: {
        from_timestamp: Joi.number().integer().description('Report period start timestamp'),
        to_timestamp: Joi.number().integer().description('Report period end timestamp'),
        count: Joi.number().integer().description('Number of top objects to report'),
        report_type: Joi.string().valid ( 'requests', 'cache_miss', 'object_not_found', '5xx_errors' ).description('Type of requested top object report'),
        country: Joi.string().description('Two-letters country code of end user location'),
      }
    },
//    response: {
