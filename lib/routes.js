  ...
  config: {
    ...
    validate: {
      ...
      query: {
        ...
        report_type: Joi.string().required().valid ( 'referer', 'status_code', 'content_type', 'protocol', 'http_protocol', 'http_method',
          'content_encoding', 'os', 'device', 'country' ).description('Type of requested report (defaults to "referer")'),
        country: Joi.string().length(2).uppercase().regex(/[A-Z]{2}/).description('Two-letters country code of end user location to filter'),
      }
    },
//    response: {
