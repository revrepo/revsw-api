...
describe('Negative check', function () {
   ...
   */
  var getEmptyCheckCallBack = function (ddCase) {
  ...
  before(function (done) {
    API.helpers
      ...
      .then(function (newDomainConfig) {
        ...
        fullDomainConfig.account_id = domainConfig.account_id;
        console.log('fullDomainConfig.account_id', fullDomainConfig.account_id);
      })
      .then(done)
      .catch(done);
  });
    ...
    describe('Update with empty data', function () {
      ...
      for (var key in dcSchema) {
        ...
        if (ddCase.skipReason) {
          ...
          xit(ddCase.spec, getEmptyCheckCallBack(ddCase));
        }
        else {
          // Running the test for specific DataDriven case
          it(ddCase.spec, getEmptyCheckCallBack(ddCase));
        }
      }
    });
  });
