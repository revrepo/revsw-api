...
describe('UsageReport Functional check:', function () {
  ...
  it( 'UsageReport contains correct billable bandwidth values', function () {
    ( Math.abs( report.billable_sent_bps -
      ( estimated.sent_bytes * 8 / test_data_timespan ) ) < 100 ).should.be.true();
    ( Math.abs( report.billable_received_bps -
      ( estimated.received_bytes * 8 / test_data_timespan ) ) < 100 ).should.be.true();
  });
