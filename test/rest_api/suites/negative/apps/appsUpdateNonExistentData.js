...
describe('Negative check', function () {
  ...
  describe('Apps resource', function () {
    describe('Update with `non-existent` data', function () {

      var testAccount;
      var testApp;
      var fullTestApp = AppsDP.generateOneForUpdate(0, 'NEW');
      ...
      var getNonExistentDataCheckCallBack = function () {
      ...
      for (var key in appsSchema) {
        ...
        var ddCase = AppsDDHelper
          .generateNonExistentData('update', key, appsSchema[key]);
        ...
        if (ddCase.skipReason) {
          ...
          xit(ddCase.spec, getNonExistentDataCheckCallBack(ddCase));
        }
        else {
          // Running the test for specific DataDriven case
          it(ddCase.spec, getNonExistentDataCheckCallBack(ddCase));
        }
      }
    });
  });
