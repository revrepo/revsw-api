...
describe('Negative check', function () {
    ...
    describe('Authenticate SSO Azure resource with Invalid Data', function () {
        ...
        it('should return a `400 Bad Request` response when authenticating with invalid `resourceId`',
            function (done) {
                API.resources.authenticateSSOAzure
                    .createOne({ token: authToken.token, resourceId: 'imInvalid' })
                    .expect(400)
                    .end(done);
            });

        it('should return a `400 Bad Request` response when authenticating with invalid `token`',
            function (done) {
                API.resources.authenticateSSOAzure
                    .createOne({ resourceId: authToken.resourceId, token: 'imInvalid' })
                    .expect(401)
                    .end(done);
            });
