  ...
  async.waterfall([
    ...
    function(cb) {
      cds_request({
        ...
        if (err) {
          return cb('CDS failed to create a new WAF Rule Generation Job');
        }
        newJobResponseJSON = JSON.parse(body);
        cb(null, res);
      });
    }
  ], function(err, res) {
    ...
    if (res.statusCode === 400) {
      return reply(boom.badRequest(newJobResponseJSON.message));
    } else if (res.statusCode === 500) {
      return reply(boom.badImplementation(newJobResponseJSON.message));
    } else if (res.statusCode === 200) {
      renderJSON(request, reply, err, newJobResponseJSON);
    } else {
