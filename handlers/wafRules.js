  ...
  async.waterfall([
    ...
    function(cb) {
      domainConfigs.get(domainId, function(error, result) { // jshint ignore:line
        ...
        if (!result || (!permissionCheck.checkPermissionsToResource(request, result, 'domains') && !permissionCheck.checkPermissionsToResource(request, result, 'security_analytics'))) {
